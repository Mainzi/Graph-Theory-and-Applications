...
class OSM_handler(osmium.SimpleHandler):
    ...
    def remove_amenities_10(self):
        good = [23, 28, 70, 58, 29, 12, 24, 40, 38, 43]
        amen = list(self.amenities)
        for i in range(0, len(self.amenities)):
            if i not in good:
                self.amenities.pop(amen[i])
...
def add_way_to_amenities(handler, amenities):
    ...
    for node in near_node:
        ...
        handler.add_adjacency_node(node, near_node[node])
    draw.save()
...
def add_start_node(handler, lat, lon, draw_node=False, id_node=0):
    ...
    handler.add_adjacency_node(id_near_node, id_node)
    if draw_node:
...
def draw_path(path, drawing, color, width, nodes):
    ...
    x, y = flip_and_center_point(nodes[path[-1]])
    drawing.add(drawing.circle((x, y), r=9, fill="gold"))
...
def test(handler, count_coord=3):
    ams = list(h.amenities)[40:50]
    print("Ways to: {0} restaurant".format(len(ams)))
    lats = []
...
# 2 задание по описанию
# с алгоритмом А*
def task_2(handler):
    lt, ln = enter_coordinates()
    add_start_node(handler, lt, ln, True)
    amen = list(handler.amenities)
    # print(handler.amenities)
    ways = {}
    for g in amen:
        ...
        draw_path(way, draw, "navy", 5, handler.nodes)
        draw.save()
        ways[int(way[-1])] = way
...
# коммивояжер методом ближайшего соседа
def nearest_neighbour(handler):
    colors = ["olive", "fuchsia", "maroon", "indigo", "lime", "purple", "green", "aqua", "teal", "blue", "sienna",
              "navy"]
    lt, ln = enter_coordinates()
    visited = set()
    add_start_node(handler, lt, ln, True, 0)
    amen = list(handler.amenities)
    current = 0
    full_distance = 0

    for i in range(0, len(amen)):
        min_way = []
        min_amen_id = -1
        min_distance = 40000
        # print(i)
        for a in amen:
            if a in visited:
                continue
            # print(str(current) + "  " + str(a))
            came, _distance, came_to_goal = a_star(handler, current, a)
            if not came_to_goal:
                print("No ways")
                continue
            way, distance = reconstruct_path(came, current, a, handler.nodes)
        ...
        if min_amen_id != -1:
            current = min_amen_id
            visited.add(min_amen_id)
            full_distance += min_distance
            draw_path(min_way, draw, colors[len(visited)], 2 + len(visited), handler.nodes)
            g = draw.g(
                style="font-size:60;font-family:Comic Sans MS, Arial;font-weight:bold;font- style:oblique;stroke:black;stroke-width:1;fill:red")
            x, y = flip_and_center_point(handler.nodes[min_amen_id])
            g.add(draw.text(str(len(visited)), insert=(x, y)))  # settings are valid for all text added to 'g'
            draw.add(g)
            # print("id = " + str(min_amen_id) + "\ndistance = " + str(full_distance))
        else:
            break
    draw.save()
    came, _distance, came_to_goal = a_star(handler, current, 0)
    if not came_to_goal:
        print("No ways")
    way, distance = reconstruct_path(came, current, 0, handler.nodes)
    full_distance += distance
    draw_path(way, draw, colors[len(visited) + 1], 2 + len(visited), handler.nodes)
    print("Full distance = {0}".format(full_distance))
    draw.save()
    del_start_node(h)


def spanning_tree(handler):
    colors = ["olive", "fuchsia", "maroon", "indigo", "lime", "purple", "green", "aqua", "teal", "blue", "sienna",
              "navy"]
    lt, ln = enter_coordinates()
    add_start_node(handler, lt, ln, True, 0)
    amen = [0]
    for a in list(handler.amenities):
        amen.append(a)
    N = len(amen)
    distance_matrix = [[float('inf')] * N for i in range(N)]
    edges = []
    for i in range(N):
        for j in range(N):
            if i == j:
                continue
            came, _distance, came_to_goal = a_star(handler, amen[i], amen[j])
            if not came_to_goal:
                print("No ways")
                continue
            way, distance = reconstruct_path(came, amen[i], amen[j], handler.nodes)
    ...
    for e in range(N-1):
        ...
        parent_id = 0
        for v in visited:
            for i, distance in enumerate(distance_matrix[v]):
                if distance < min_len and i not in visited:
                    min_len = distance
                    min_id = i
                    parent_id = v
        if parent_id not in span_tree.keys():
            span_tree[parent_id] = []
        span_tree[parent_id].append(min_id)
        visited.add(min_id)
    # span_tree - словарь ребер остовного дерева

    def down(n, path):
        path.append(n)
        if n not in span_tree.keys():
            return

        for child in span_tree[n]:
            down(child, path)
        return

    salesman_path = []

    down(0, salesman_path)
    salesman_path.append(0)

    # рисуем и считаем дистанцию
    full_distance = 0
    for i in range(1, len(salesman_path)):
        came, _distance, came_to_goal = a_star(handler, amen[salesman_path[i-1]], amen[salesman_path[i]])
        if not came_to_goal:
            print("No ways")
            continue
        way, distance = reconstruct_path(came, amen[salesman_path[i-1]], amen[salesman_path[i]], handler.nodes)
        full_distance += distance
        draw_path(way, draw, colors[i], 2 + i, handler.nodes)
        g = draw.g(
            style="font-size:60;font-family:Comic Sans MS, Arial;font-weight:bold;font- style:oblique;stroke:black;stroke-width:1;fill:red")
        x, y = flip_and_center_point(handler.nodes[amen[salesman_path[i]]])
        g.add(draw.text(str(i), insert=(x, y)))  # settings are valid for all text added to 'g'
        draw.add(g)
    draw.save()
    print(salesman_path)
    print("Full distance = {0}".format(full_distance))
...
if __name__ == '__main__':
    ...
    h.amenities = h.remove_amenities_10()
